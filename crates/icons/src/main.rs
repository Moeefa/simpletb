...
pub fn get_icon(ext: &str, size: i32) -> Result<Vec<u8>, &'static str> {
  ...
  unsafe {
    if ext != "" {
      let mut path = PathBuf::from(&ext).parent().unwrap().to_path_buf();
      path.push("assets");
      path.push("DesktopShortcut.ico");
    ...
    let im = image::load_from_memory(&bytes).expect("Failed to load image");
    let mut png_bytes: Vec<u8> = Vec::new();
    im.write_to(&mut Cursor::new(&mut png_bytes), ImageFormat::Png)
