...
use ui::ScreenGeometry;
use ui::AppBar;
use ui::Dock;
use ui::Hitbox;
use ui::MARGIN_BOTTOM;
...
async fn open_settings(app: tauri::AppHandle) {
  ...
  if window == None {
    let _ = tauri::WebviewWindowBuilder::new(
      &app,
      "settings",
      tauri::WebviewUrl::App("/crates/ui/src/settings/index.html".into()),
    )
      .title("Settings")
      .resizable(false)
...
fn setup_tauri_app(
  ...
  return app_builder.setup(move |app| {
    ...
    Hitbox::new(app.app_handle().clone())
      .init()
      .expect("Failed to initialize hitbox");

    let dock = Dock::new(app.app_handle().clone());
...
fn create_round_window(
  ...
  let webview_window = tauri::WebviewWindowBuilder::new(
    &app,
    "round-border",
    tauri::WebviewUrl::App("/crates/ui/src/menubar/rounded/index.html".into()),
  )
  .title("rounded")
  .decorations(false)
  .resizable(false)
